...
public class TransactionHandler(AppDbContext context) : ITransactionHandler
{
    public async Task<Response<Transaction?>> CreateAsync(CreateTransactionRequest request)
    {
        ...
        try
        {
            var transaction = new Transaction
            {
                ...
                ? DateTime.SpecifyKind(request.PaidOrReceivedAt.Value, DateTimeKind.Local).ToUniversalTime()
                : null,
                Title = request.Title,
                Type = request.Type
            };
    ...
    public async Task<PagedResponse<List<Transaction>?>> GetByPeriodAsync(GetTransactionsByPeriodRequest request)
    {
        try
        {
            // Use the provided dates if available, otherwise use current month
            request.StartDate = request.StartDate?.ToUniversalTime() ?? DateTime.UtcNow.GetFirstDay();
            request.EndDate = request.EndDate?.ToUniversalTime() ?? DateTime.UtcNow.GetLastDay();
        }
        catch
        {
